...
def change_numbers():
    file = open("DailyStella\pic_number.txt", "r")
    pic_number = int(file.read())
    if date_is_different():
        pic_number += 1
        file.close()
        file = open("DailyStella\pic_number.txt", "w")
        file.write(str(pic_number))
        file.close()
        file = open("DailyStella/last_update.txt", "w")
...
def date_is_different():
    ...
    file = open("DailyStella\last_update.txt", "r")
